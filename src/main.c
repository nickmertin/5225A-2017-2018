...
bool gVelDirCheck  = false;
#define VEL_DIR_CHECK_TRUE unsigned long timeStart = nPgmTime; \
if (nPgmTime - timeStart > 200) gVelDirCheck = 1;
...
void liftRaiseSimple(int target, word mainPower, word brakePower)
{
	...
	setLift(mainPower);
	int pos;
	while ((pos = gSensor[liftPoti].value) < target)
	{
		VEL_DIR_CHECK_TRUE
		sleep(10);
	}
