/* Functions */
#define NOT_SAFETY(message) !TimedOut(npgmTime + 100, TID1(message, 0), true, VEL_LOCAL_Y, 0.5, nPgmTime-timeStart, &velSafetyCounter)
void moveToTargetSimple(float y, float x, float ys, float xs, byte power, byte startPower, float maxErrX, float decelEarly, byte decelPower, float dropEarly, tStopType stopType, tMttMode mode, bool velSafety)
{
	...
	do
	{
		...
		endCycle(cycle);
	} while (currentPosVector.y < -dropEarly - MAX((vel * ((stopType & stopSoft) ? 0.175 : 0.098)), decelEarly) && (velSafety? NOT_SAFETY(moveToTargetSimple) : 1 ) );
...
void turnToAngleNewAlg(float a, tTurnDir turnDir, float fullRatio, byte coastPower, float stopOffsetDeg, bool mogo, bool harshStop, bool velSafety)
{
	...
	case cw:
		...
		setDrive(127, -127);
		while (gPosition.a < endFull && (velSafety? NOT_SAFETY(turnToAngleNewAlg-full) : 1 ))
		{
			if (DATALOG_TURN != -1)
			{
				tHog();
...
void turnToTargetNewAlg(float y, float x, tTurnDir turnDir, float fullRatio, byte coastPower, float stopOffsetDeg, bool mogo, bool harshStop, float offset, bool velSafety)
{
	...
	case cw:
		...
		setDrive(127, -127);
		while (gPosition.a < endFull && (velSafety? NOT_SAFETY(turnToTargetNewAlg-full) : 1 ))
		{
			if (DATALOG_TURN != -1)
			{
				tHog();
		...
		setDrive(coastPower, -coastPower);
		timeStart = npgmTime;
		while (gPosition.a < nearAngle(atan2(x - gPosition.x, y - gPosition.y) + offset, target) - degToRad(stopOffsetDeg) && (velSafety? NOT_SAFETY(turnToTargetNewAlg-coast) : 1 ))
		{
			if (DATALOG_TURN != -1)
			{
				tHog();
	...
	case ccw:
		...
		setDrive(-coastPower, coastPower);
		timeStart = nPgmTime;
		while (gPosition.a > nearAngle(atan2(x - gPosition.x, y - gPosition.y) + offset, target) + degToRad(stopOffsetDeg) && (velSafety? NOT_SAFETY(turnToTargetNewAlg-full) : 1 ))
		{
			if (DATALOG_TURN != -1)
			{
				tHog();
...
void sweepTurnToTarget(float y, float x, float a, float r, tTurnDir turnDir, byte power, bool slow, bool velSafety)
{
	...
	case cw:
		...
		do
		{
			...
			endCycle(cycle);
		} while ((power > 0 ? gPosition.a : (gPosition.a + PI)) - a < (slow ? -0.1 : -0.15) && (velSafety? NOT_SAFETY(sweepTurnToTarget) : 1 ));
		break;
	case ccw:
		vector.y = 0;
		vector.x = r;
		...
		do
		{
			...
			endCycle(cycle);
		} while ((power > 0 ? gPosition.a : (gPosition.a + PI)) - a > (slow ? 0.1 : 0.15) && (velSafety? NOT_SAFETY(moveToTargetSimple) : 1));
		break;
	}
	setDrive(0, 0);
