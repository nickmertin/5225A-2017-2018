...
void updateSensorInput(tSensors sen)
{
	...
	if (SensorType[sen] == sensorPotentiometer && ++gSensor[sen].filterAcc < 10 && ( (abs(gSensor[sen].value - gSensor[sen].lstValue) > 400) || (gSensor[sen].velGood && gVelDirCheck && sgn(gSensor[sen].velocity) != sgn(gSensor[sen].lstVelocity)) ) )
	{
